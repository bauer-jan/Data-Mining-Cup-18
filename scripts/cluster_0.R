...
train <- train %>% mutate(cluster=ifelse(number_of_sales_per_month > 1 & number_of_sales_per_month <4, 0,  1))
test <- test %>% mutate(cluster=ifelse(number_of_sales_per_month <= 1.2, 0,  1))

train <- train %>% filter(cluster==0)
test <- test %>% filter(cluster==0)
...
y_hat_rf_0<- as.numeric(as.data.frame(h2o.predict(mod_rf, h2o_test))$predict)
...
y_hat_xg_0<- as.numeric(as.data.frame(h2o.predict(mod_xg, h2o_test))$predict)
...
y_hat_glm_0 <- as.numeric(as.data.frame(h2o.predict(mod_glm, h2o_test))$predict)
...
#BAGGING
cluster_0 <- (y_hat_xg_0+y_hat_rf_0+y_hat_glm_0)/3
...
#final <- cbind(test[,c("pid","size","date","unitsSold")],cluster_0)
final <- cbind(test[,c("pid","size","date")],cluster_0)
final <- cbind(final,y_hat_rf_0)
final <- cbind(final,y_hat_xg_0)
final <- cbind(final,y_hat_glm_0)
...
write.table(items,"final_submission/cluster_0.csv",row.names = F,quote = F,sep = "|")
